...
void CalcPointLightContribution(vec3 viewPos, vec3 normal, Light light, float shininess, inout vec3 diffuse, inout vec3 specular) {
      ...
      //  if(IsFlagSet(FLAG_ENABLE_COLOR_DIFFUSE_WARP))
      //  {
     //       vec3 diffuseWarp = texture(s_DiffuseRamp,NdotL).rgb;
   //         diffuse += attenuation * light.PositionIntensity.w * light.ColorAttenuation.rgb * diffuseWarp;
   //     }
   //     else
   //     {
  ...
  //      }
     ...
     //   if(IsFlagSet(FLAG_ENABLE_COLOR_SPECULAR_WARP))
   //     {
   //         vec3 specWrap = texture(s_SpecularRamp, VdotR).rgb;
  //          specWarp = pow(specWarp, pow(2,shininess *8));
  //      }
   //     else
  //      {
